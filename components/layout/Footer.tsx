        ...
        <footer className="bg-furniture-muted border-t mt-auto">
            <div className="container mx-auto px-4 py-12 sm:px-6 lg:px-8">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                    <div>
                        <h3 className="font-serif text-xl font-bold mb-4">{settings.companyName || siteConfig.name}</h3>
                    ...
                    <div>
                        ...
                        <ul className="space-y-2 text-sm text-muted-foreground">
                            <li>{settings.address || "İstanbul, Türkiye"}</li>
                            {settings.email && <li><a href={`mailto:${settings.email}`} className="hover:text-furniture-bronze">{settings.email}</a></li>}
                            {settings.phone && <li><a href={`tel:${settings.phone}`} className="hover:text-furniture-bronze">{settings.phone}</a></li>}
                        </ul>
                    </div>
                </div>
                <div className="mt-8 border-t pt-8 flex flex-col md:flex-row justify-between items-center">
                    <p className="text-sm text-muted-foreground">
                        &copy; {new Date().getFullYear()} {settings.companyName || siteConfig.name}. Tüm hakları saklıdır.
                    </p>
